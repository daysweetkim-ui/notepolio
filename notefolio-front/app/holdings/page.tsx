...
  return (
    <div className="bg-white border border-slate-200 shadow-sm hover:shadow-md rounded-3xl p-5 mb-4">
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1 cursor-pointer" onClick={() => onTradeClick(holding)}>
          <div className="flex items-center gap-2">
            <span className="font-extrabold text-lg text-slate-900 tracking-tight">{holding.name ?? holding.ticker}</span>
            {pc?.change_pct != null && (
              <span className={`text-xs px-2 py-0.5 bg-slate-50 rounded-md border border-slate-100 ${pnlColor(pc.change_pct)}`}>
                {fmtPct(pc.change_pct)}
              </span>
            )}
          </div>
          <div className="flex items-center gap-2 mt-1">
            <p className="text-xs font-bold text-slate-400">{holding.ticker}</p>
            {accountName && (
              <>
                <span className="text-slate-300">|</span>
                <span className="text-xs font-bold text-violet-600">{accountName}</span>
              </>
            )}
          </div>
        </div>
...
          <div className="text-right cursor-pointer" onClick={() => onTradeClick(holding)}>
            <p className="font-black text-base text-slate-900">{fmtPrice(evalValue, displayCurrency, exchangeRate)}</p>
            <p className={`text-xs mt-0.5 ${pnlColor(totalPnlPct)}`}>{totalPnlPct != null ? fmtPct(totalPnlPct) : "—"}</p>
          </div>
          <button onClick={() => setShowActions(!showActions)} className="text-slate-400 hover:text-slate-700 bg-slate-50 p-1.5 rounded-full transition-colors">
...
            매매 / 히스토리
          </button>
          <button onClick={() => { setShowActions(false); setShowDeleteConfirm(true) }} className="px-4 py-2.5 bg-red-50 hover:bg-red-100 transition-colors text-red-600 rounded-xl text-xs font-bold">
            삭제
          </button>
        </div>
      )}
...
      {showDeleteConfirm && (
        <div className="mb-4 bg-red-50 p-3 rounded-xl flex items-center justify-between border border-red-100">
          <p className="text-xs font-bold text-red-600">정말 포지션을 완전히 삭제할까요?</p>
          <div className="flex gap-2">
            <button onClick={() => setShowDeleteConfirm(false)} className="text-xs font-bold text-slate-500 hover:text-slate-700">취소</button>
...
        ].map((item) => (
          <div key={item.label} className="bg-slate-50 border border-slate-100 rounded-xl px-3 py-2.5">
...
                setSearching(false)
              }}
              className="bg-violet-600 text-white px-4 rounded-xl text-xs font-bold shadow-sm"
            >
              {searching ? "..." : "검색"}
            </button>
          </div>
...
              <span>선택됨: {selected.ticker}</span>
              <button onClick={() => setSelected(null)} className="hover:text-red-500 transition-colors"><X size={16} /></button>
            </div>
          )}
          
          <div>
...
  const [showCashModal, setShowCashModal] = useState(false)     // 💡 현금 모달 상태 복구
  const [showAccountModal, setShowAccountModal] = useState(false) // 💡 계좌 관리 모달 상태 복구
...
            className="flex items-center gap-1 bg-white border border-slate-200 px-3 py-2 rounded-xl text-xs font-bold text-slate-500 shadow-sm hover:bg-slate-50 transition-colors"
          >
            {displayCurrency === "KRW" ? <Banknote size={14} /> : <DollarSign size={14} />}
            <span className="hidden sm:inline">{displayCurrency === "KRW" ? "원화 보기" : "달러 보기"}</span>
          </button>
          
          {/* 💡 누락되었던 계좌 관리 버튼 복구 */}
          <button
            onClick={() => setShowAccountModal(true)}
            className="flex items-center gap-1.5 bg-slate-100 hover:bg-slate-200 px-3 py-2 rounded-xl text-xs font-bold transition-colors text-slate-700"
          >
            <Wallet size={14} />
            <span className="hidden sm:inline">계좌 관리</span>
          </button>

          {/* 종목 추가 버튼 */}
          <button
            onClick={() => setShowAddModal(true)}
            className="flex items-center gap-1 bg-violet-600 hover:bg-violet-700 text-white px-3 py-2 rounded-xl text-xs font-bold shadow-sm transition-colors"
          >
            <Plus size={14} />
...
          className={`shrink-0 px-5 py-2 rounded-full text-xs font-bold border transition-colors ${
